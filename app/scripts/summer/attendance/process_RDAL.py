...
def main(form, request):
    ...
    f = process_rdal_csv_and_save(filename,class_date)
...
def process_rdal_csv_and_save(filename,class_date):
    ...
    path = os.path.join(
        current_app.root_path, f"data/{year_and_semester}/RDAL"
    )
    ...
    rdal_df = rdal_df.rename(columns={
    ...
    rdal_df[['Student Last Name', 'First Name','Phone', 'Student DOE Email']].to_csv(f, index=False)
