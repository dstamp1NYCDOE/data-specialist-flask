...
def calculate_relative_risk(df, outcome_col, group_col):
    """
    Calculate relative risk of outcome across different groups.
    Sorted by highest relative risk first.
    ...
    df : pd.DataFrame
        Dataset
    outcome_col : str
        Binary outcome column (e.g., Profile_A)
    group_col : str
        Grouping variable (e.g., Counselor, year_in_hs)
        
    Returns:
    --------
    list of dict
        Relative risk calculations for each group comparison, sorted by RR descending
    """
    results = []
    ...
    for group in groups:
        ...
        n_outcome = group_df[outcome_col].sum() if outcome_col in group_df.columns else 0
        risk = n_outcome / n_total if n_total > 0 else 0
    ...
    # Use group with lowest risk as reference for more intuitive RR > 1
    reference_group = min(groups, key=lambda g: group_risks[g])
    reference_risk = group_risks[reference_group]
    
    # Calculate relative risk for each comparison group
    for group in groups:
        if group == reference_group:
            continue
        
        comparison_risk = group_risks[group]
        
        # Calculate relative risk (avoid division by zero)
        if reference_risk > 0:
            relative_risk = comparison_risk / reference_risk
        else:
        ...
        results.append({
            ...
            'reference_n': group_counts[reference_group]['total'],
            'comparison_n': group_counts[group]['total'],
            'reference_rate': reference_risk,
            'comparison_rate': comparison_risk
        })
    
    # Sort by relative risk (highest first)
    results.sort(key=lambda x: x['relative_risk'] if not pd.isna(x['relative_risk']) else 0, reverse=True)
