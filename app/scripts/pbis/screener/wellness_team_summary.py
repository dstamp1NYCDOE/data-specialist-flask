...
def return_download_file(df):
    ...
    sheets.append((f'{cohort}', students_df[cols]))

    sheets.append((f'{cohort}_summary', return_questions_pvt(cohort_students_df)))

    for counselor, counselor_students_df in students_df.groupby('Counselor'):
        top_10_bottom_10_df = pd.concat([counselor_students_df.head(10), counselor_students_df.tail(10)])
        sheets.append((f'{counselor}', top_10_bottom_10_df[cols]))
...
def insert_student_survey_questions_chart(writer, sheet):
    ...
    cols = ['left_buffer', 'Strongly Agree', 'Agree', 'Disagree', 'Strongly Disagree', 'right_buffer']
    fill_colors = [{'none': True}, {'color': '#118ab2'}, {'color': '#06d6a0'}, {'color': '#ffd166'}, {'color': '#ef476f'},{'none': True}]
    patterns = []
    letters = ['B','C','D','E','F','G']
    custom_label_letters = ['', 'H', 'I', 'J', 'K', '']
    custom_label_letters = ['','C','D','E','F','']
    i = 11
    for col, letter, fill_color, custom_letter in zip(cols, letters, fill_colors, custom_label_letters):
        custom_labels = [
    ...
    diverging_bar_chart.set_legend(
        {'position':'bottom','font': {'size': 12, 'bold': True, 'name': 'Helvetica Neue'}, 'delete_series': [0, 5]})
    
    
    diverging_bar_chart.set_y_axis(
        {'name_font': {'size': 14, 'bold': True, 'name': 'Helvetica Neue'}})
    
    diverging_bar_chart.set_x_axis({'visible': False, })
...
def return_questions_pvt(df):
    ...
    cat_type = CategoricalDtype(categories=["Strongly Disagree","Disagree","Agree","Strongly Agree"], ordered=True)
    ...
    student_pvt['left_buffer'] = student_pvt['Agree'] +student_pvt['Strongly Agree'] 
    student_pvt['right_buffer'] = student_pvt['Disagree'] +student_pvt['Strongly Disagree'] 

    student_pvt = student_pvt[['left_buffer', 'Strongly Disagree', 'Disagree', 'Agree', 'Strongly Agree', 'right_buffer']]
