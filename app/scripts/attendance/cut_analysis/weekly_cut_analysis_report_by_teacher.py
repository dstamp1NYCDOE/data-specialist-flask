...
def main(form, request):
    ...
    student_cuts_by_period = (
        student_period_attendance_df[student_period_attendance_df["cutting?"]]
        .groupby(["StudentID", "Pd", "Course"])["Date"]
        .apply(list)
    )
    student_cuts_by_period = (
        pd.DataFrame(student_cuts_by_period)
        .rename(columns={"Date": "Dates Cut"})
        .reset_index()
    )

    cuts_df = student_period_attendance_df[
        student_period_attendance_df["cutting?"]
    ].sort_values(by=["Teacher", "Pd", "LastName", "FirstName"])
    cuts_df = cuts_df.merge(
        student_cuts_by_period, on=["StudentID", "Pd", "Course"], how="left"
    )
    cuts_df["Dates Cut"] = cuts_df["Dates Cut"].apply(
        lambda x: ", ".join([y[5:] for y in x])
    )

    cuts_df = cuts_df.drop_duplicates(
        subset=[
            "StudentID",
            "Course",
            "Section",
    ...
    late_to_school_df = student_period_attendance_df[
        student_period_attendance_df["Type"].isin(["unexcused", "tardy"])
    ]
    student_late_to_school_by_period = (
        late_to_school_df[late_to_school_df["late_to_school?"]]
        .groupby(["StudentID", "Pd", "Course"])["Date"]
        .apply(list)
    )
    student_late_to_school_by_period = (
        pd.DataFrame(student_late_to_school_by_period)
        .rename(columns={"Date": "Dates Late to school"})
        .reset_index()
    )

    late_to_school_df = late_to_school_df[
        late_to_school_df["late_to_school?"]
    ].sort_values(by=["Teacher", "Pd", "LastName", "FirstName"])
    late_to_school_df = late_to_school_df.merge(
        student_late_to_school_by_period, on=["StudentID", "Pd", "Course"], how="left"
    )
    late_to_school_df["Dates Late to school"] = late_to_school_df[
        "Dates Late to school"
    ].apply(lambda x: ", ".join([y[5:] for y in x]))

    late_to_school_df = late_to_school_df.drop_duplicates(
        subset=[
            "StudentID",
            "Course",
            "Section",
    ...
    attd_errors_df = student_period_attendance_df[
        student_period_attendance_df["attd_error"]
    ]
    teachers = student_period_attendance_df["Teacher"].sort_values().unique().tolist()
    for teacher in teachers:

        students_present_in_one_period = attd_errors_df[
            attd_errors_df["Teacher"] == teacher
        ]
        students_with_cuts_df = cuts_df[cuts_df["Teacher"] == teacher]
        students_absent_all_week = students_absent_all_week_df[
            students_absent_all_week_df["Teacher"] == teacher
        ].drop_duplicates(subset=["StudentID"])

        students_with_lates_df = late_to_school_df[
            late_to_school_df["Teacher"] == teacher
        ]

        days_with_attendance_by_teacher = student_period_attendance_df[
            student_period_attendance_df["Teacher"] == teacher
        ]["Date"].unique()

        teacher_flowables = return_flowables_by_teacher(
            students_with_cuts_df,
            students_with_lates_df,
            students_present_in_one_period,
            students_absent_all_week,
            teacher,
            styles,
            date_of_report,
            days_with_attendance_by_teacher,
            dates_covered_by_report,
        )
        flowables.extend(teacher_flowables)

    lates_to_school_df = student_period_attendance_df[
        student_period_attendance_df["late_to_school?"]
    ]
    lates_to_school_df = lates_to_school_df.drop_duplicates(
        subset=["StudentID", "Date"]
    )
    lates_to_school_df = lates_to_school_df[
        lates_to_school_df["num_of_late_to_school"] >= num_of_days
    ]

    student_days_late = lates_to_school_df.groupby(["StudentID"])["Date"].apply(list)
    student_days_late = (
        pd.DataFrame(student_days_late)
        .rename(columns={"Date": "Dates Late"})
        .reset_index()
    )
    ...
    lates_to_school_df = lates_to_school_df.merge(
        student_days_late, on=["StudentID"], how="left"
    )
    lates_to_school_df = lates_to_school_df.sort_values(
        by=["Dates Late", "LastName", "FirstName"]
    )
    ...
    for (cohort_year,), students_with_lates_df in group_by_cohort:
        flowables.extend(
            return_flowables_overall_late_to_school(
                cohort_year, students_with_lates_df, styles, date_of_report
            )
        )

    overall_cuts_df = cuts_df[cuts_df["num_of_cuts"] >= num_of_days].drop_duplicates(
        subset=["StudentID"]
    )
    overall_cuts_df = overall_cuts_df.sort_values(
        by=["num_of_cuts", "LastName", "FirstName"], ascending=[False, True, True]
    )
    group_by_cohort = overall_cuts_df.groupby(["year_in_hs"])

    for (cohort_year,), students_with_cuts in group_by_cohort:
        flowables.extend(
            return_flowables_overall_cuts_to_school(
    ...
    for (cohort_year,), students_absent_all_week_by_cohort in group_by_cohort:
        flowables.extend(
            return_flowables_overall_absent_all_week_to_school(
...
def return_flowables_by_teacher(
    students_with_cuts_df,
    students_with_lates_df,
    students_present_in_one_period,
    students_absent_all_week,
    ...
    paragraph = Paragraph(f"{teacher}", styles["Body_Justify"])
...
def return_flowables_overall_late_to_school(
    cohort_year, lates_to_school_df, styles, date_of_report
):
    temp_list_of_flowables = []
    paragraph = Paragraph(f"{cohort_year}", styles["Body_Justify"])
    temp_list_of_flowables.append(paragraph)
    temp_list_of_flowables.append(Spacer(width=0, height=0.25 * inch))
...
def return_flowables_overall_cuts_to_school(
    cohort_year, students_with_cuts, styles, date_of_report
):
    temp_list_of_flowables = []
    paragraph = Paragraph(f"{cohort_year}", styles["Body_Justify"])
