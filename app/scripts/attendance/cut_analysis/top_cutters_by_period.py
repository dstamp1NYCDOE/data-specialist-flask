...
def main():
    ...
    total_cuts_by_student = pd.pivot_table(attd_by_student, index=['StudentID','FirstName','LastName'],values=True,aggfunc='sum')
    ...
    for period in [1,2,3,4,5,6,7,8,9]:
        ...
        top_30_lates = attd_by_student[attd_by_student['Pd']==period].sort_values(by=[True]).drop_duplicates(subset=['StudentID']).tail(27)
        flowables.extend(return_photo_roster_pdf(page_header, top_30_lates))

    total_cuts_by_student = total_cuts_by_student.reset_index()
    total_cuts_by_student = total_cuts_by_student.merge(photos_df, on=['StudentID'], how='left')
    page_header = f"Top Cuts Overall"
    top_27_cuts = total_cuts_by_student.sort_values(by=[True]).drop_duplicates(subset=['StudentID']).tail(27)
