...
def main():
    ...
    for year_in_hs, _df in students_df.groupby("year_in_hs"):
        _dff = _df.sort_values(
            by=["%_of_classes_passing", "Counselor"], ascending=[True, True]
        )
        _dff.to_excel(writer, sheet_name=f"{year_in_hs}", index=False)

    for num_passing in [0]:
        _dff = students_df[
            students_df["#_of_classes_passing"] == num_passing
        ].sort_values(by=["%_of_classes_passing", "Counselor"], ascending=[True, True])
        _dff.to_excel(writer, sheet_name=f"{num_passing}_classes_passing", index=False)

    for num_failing in [1]:
        _dff = students_df[
            students_df["#_of_classes_failing"] == num_failing
        ].sort_values(by=["%_of_classes_passing", "Counselor"], ascending=[True, True])
        _dff.to_excel(writer, sheet_name=f"{num_failing}_classes_failing", index=False)

    writer.close()
    f.seek(0)

    download_name = f"JupiterGradesStudentCheckup-{year_and_semester}-{dt.datetime.now().strftime('%Y_%m_%d')}.xlsx"
