...
def main():
    ...
    stats_df = pd.pivot_table( 
        cr_1_14_df, index=['Year','Term','Curriculum'], values='NumericEquivalent', aggfunc=('mean','std')
    ...
    student_stats_by_term = pd.pivot_table( 
        cr_1_14_df, index=['StudentID','Year','Term'], values='z-score',aggfunc='mean'
    ).reset_index()
    ...
    output_cols = [
        ...
        'most_recent_term','z-score_net_gain',"z-score_total",'sparkline'
    ]
