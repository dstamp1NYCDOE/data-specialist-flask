...
def main(form, request):
    ...
    grades_df["final_mark_bin"] = pd.cut(
        grades_df["FinalMark"], bins=bins, labels=labels, right=True
    )
    ...
    for teacher_col in ["Teacher", "Co-Teacher"]:
        section_properties_df[teacher_col] = section_properties_df[
            teacher_col
        ].str.strip()
    ...
    attach_co_teacher_df = attach_co_teacher_df.rename(
        columns={"EmployeeID": "EmployeeID_2", "Teacher": "Co-Teacher"}
    )
    ...
    dff["teacher_of_interest"] = (dff["EmployeeID_1"] == teacher_of_interest) | (
        dff["EmployeeID_2"] == teacher_of_interest
    )

    dff["RowLabel"] = (
        dff["Course"].astype(str)
        + "/"
        + dff["Section"].astype(str)
        + " - "
        + dff["Teacher"]
    )
    ...
    grades_bin_pvt.to_excel(writer, sheet_name="Grades", index=True)
    writer.close()
    f.seek(0)
