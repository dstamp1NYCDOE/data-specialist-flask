...
def return_dfs_dict(marking_period):
    ...
    jupiter_period_attendance_df = return_df_by_title("jupiter_period_attendance", year_and_semester)
    
    # Pre-compute pivot tables
    jupiter_attd_summary_df = return_jupiter_attd_pvt(jupiter_period_attendance_df)
    jupiter_missing_assignment_pvt = return_jupiter_assignment_missing_pvt(assignments_df)
    
    # OPTIMIZATION: Pre-group dataframes by StudentID for fast lookup
    # Convert grouped objects to dictionaries for faster access
    grades_by_student = {student_id: group for student_id, group in grades_df.groupby('StudentID')}
    attd_by_student = {student_id: group for student_id, group in jupiter_attd_summary_df.groupby('StudentID')}
    missing_by_student = {student_id: group for student_id, group in jupiter_missing_assignment_pvt.groupby('StudentID')}
    ratr_by_student = {student_id: group for student_id, group in RATR_df.groupby('StudentID')}
    ratr_summary_by_student = {student_id: group for student_id, group in RATR_Summary_df.groupby('StudentID')}
    attd_benchmark_by_student = {student_id: group for student_id, group in attd_benchmark_df.groupby('StudentID')}
    smartpass_by_student = {student_id: group for student_id, group in smartpass_df.groupby('StudentID')}

    dfs_dict = {
        "1_01": grades_df,
        "HonorRoll": return_df_by_title("HonorRoll", year_and_semester),
        "assignments_df": assignments_df,
        "jupiter_period_attendance_df": jupiter_period_attendance_df,
        ...
        # Add pre-grouped dictionaries for fast lookups
        "grades_by_student": grades_by_student,
        "attd_by_student": attd_by_student,
        "missing_by_student": missing_by_student,
        "ratr_by_student": ratr_by_student,
        "ratr_summary_by_student": ratr_summary_by_student,
        "attd_benchmark_by_student": attd_benchmark_by_student,
        "smartpass_by_student": smartpass_by_student,
    }
...
def return_jupiter_assignment_missing_pvt(assignments_df):
...
def return_jupiter_attd_pvt(jupiter_period_attendance_df):
    # remove released days
    df = jupiter_period_attendance_df[jupiter_period_attendance_df['Attendance']!='R'].copy()

    df["ATTD"] = df["Type"].apply(swap_attd_marks_on_jupiter)
...
def generate_letters(students_df, dfs_dict, marking_period):
    # OPTIMIZATION: Cache for images to avoid regenerating identical charts
    image_cache = {}
    dfs_dict['image_cache'] = image_cache
...
def return_student_class_flowables(student_row, dfs_dict, marking_period):
    ...
    # OPTIMIZATION: Use pre-grouped dictionary instead of filtering entire dataframe
    grades_by_student = dfs_dict["grades_by_student"]
    
    if StudentID not in grades_by_student:
        return class_flowables
    
    student_classes_df = grades_by_student[StudentID].copy()
    ...
    if len(student_classes_df) > 0:
        student_classes_df["flowables"] = student_classes_df.apply(
            return_class_flowables, axis=1, args=(dfs_dict, marking_period, StudentID)
        )
...
def return_class_flowables(student_row, dfs_dict, marking_period, StudentID):
    ...
    # OPTIMIZATION: Use pre-grouped dictionary for missing assignments
    missing_by_student = dfs_dict["missing_by_student"]
    
    num_of_missing_practice = 0
    total_practice = 0
    num_of_missing_performance = 0
    total_performance = 0
    has_practice = False
    has_performance = False
    
    if StudentID in missing_by_student:
        student_missing = missing_by_student[StudentID]
        student_missing = student_missing[student_missing["Course"] == JupiterCourseCode]
        
        if len(student_missing) > 0:
            practice_rows = student_missing[student_missing["Category"] == "Practice"]
            if len(practice_rows) > 0:
                has_practice = True
                num_of_missing_practice = practice_rows["Y"].max()
                total_practice = practice_rows["All"].max()
            
            performance_rows = student_missing[student_missing["Category"] == "Performance"]
            if len(performance_rows) > 0:
                has_performance = True
                num_of_missing_performance = performance_rows["Y"].max()
                total_performance = performance_rows["All"].max()

    if num_of_missing_practice == 0 and has_practice:
    ...
    if num_of_missing_performance == 0 and has_performance:
        performance_str = f"Completed all {total_performance:.0f} performance assignments"
    ...
    ## jupiter attendance - OPTIMIZATION: Use pre-grouped dictionary and cache images
    attd_by_student = dfs_dict["attd_by_student"]
    image_cache = dfs_dict["image_cache"]
    
    I = ""
    if StudentID in attd_by_student:
        student_attd = attd_by_student[StudentID]
        student_attd = student_attd[student_attd["Course"] == JupiterCourseCode]
        
        if len(student_attd) > 0:
            # Create a cache key based on attendance values
            present = student_attd.get('present', pd.Series([0])).iloc[0] if 'present' in student_attd.columns else 0
            late = student_attd.get('late', pd.Series([0])).iloc[0] if 'late' in student_attd.columns else 0
            absent = student_attd.get('absent', pd.Series([0])).iloc[0] if 'absent' in student_attd.columns else 0
            cache_key = f"attd_{present}_{late}_{absent}"
            
            if cache_key in image_cache:
                I = image_cache[cache_key]
            else:
                I = return_jupiter_attd_summary_graph(student_attd)
                image_cache[cache_key] = I
...
def return_left_flowables(student_row, dfs_dict, marking_period):
    ...
    # OPTIMIZATION: Use pre-grouped dictionaries
    ratr_by_student = dfs_dict["ratr_by_student"]
    ratr_summary_by_student = dfs_dict["ratr_summary_by_student"]
    image_cache = dfs_dict["image_cache"]

    student_RATR_df = ratr_by_student.get(StudentID, pd.DataFrame())
    
    if StudentID in ratr_summary_by_student:
        student_RATR_summary_df = ratr_summary_by_student[StudentID]
        
        # Create cache key for daily attendance chart
        total_days = len(student_RATR_df)
        p_days = len(student_RATR_df[student_RATR_df["ATTD"] == "P"]) if len(student_RATR_df) > 0 else 0
        l_days = len(student_RATR_df[student_RATR_df["ATTD"] == "L"]) if len(student_RATR_df) > 0 else 0
        a_days = len(student_RATR_df[student_RATR_df["ATTD"] == "A"]) if len(student_RATR_df) > 0 else 0
        cache_key = f"daily_{p_days}_{l_days}_{a_days}"
    ...
    # OPTIMIZATION: Use pre-grouped dictionary for attendance benchmark
    attd_benchmark_by_student = dfs_dict["attd_benchmark_by_student"]
    attd_benchmark = False
    if StudentID in attd_benchmark_by_student:
        attd_benchmark_df = attd_benchmark_by_student[StudentID]
    ...
    # OPTIMIZATION: Use pre-grouped dictionary for smartpass
    smartpass_by_student = dfs_dict["smartpass_by_student"]
    
    if StudentID in smartpass_by_student:
        student_smartpass_df = smartpass_by_student[StudentID]
        ...
        if len(student_smartpass_df) > 0:
            ...
            # Cache smartpass charts too
            total_seconds = int(student_smartpass_df.iloc[0]["Total"])
            cache_key = f"smartpass_{StudentID}_{total_seconds}"
