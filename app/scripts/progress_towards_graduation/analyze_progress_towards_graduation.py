...
def return_spreadsheet(processed_1_68_df):
    ...
    for credit_area in credit_areas:
        ...
        df = processed_1_68_df[
            processed_1_68_df[f"{credit_area}-EnrolledToCloseGap"] == False
        ]
...
def process_1_68():
    ...
    df["max_possible_credits_to_recover"] = df["year_in_hs"].apply(
        return_num_of_makeup_credit, args=(term,)
    )
    ...
    for credit_area in credit_areas:
        ...
        df[f"{credit_area}-On_Track_Needed"] = df["year_in_hs"].apply(
            return_on_track_credits_by_year_in_hs, args=(term, credit_area)
        )
        df[f"{credit_area}-On_Track_Needed_by_end_of_term"] = df["year_in_hs"].apply(
            return_on_track_credits_by_year_in_hs, args=(term + 1, credit_area)
        )
        df[f"{credit_area}-Deficiency"] = df.apply(
            return_deficiency, args=(credit_area,), axis=1
        )
        df[f"{credit_area}-EnrolledToCloseGap"] = (
            df[f"{credit_area}-Enrolled"] >= df[f"{credit_area}-Deficiency"]
        )

    df["AreasOnTrack"] = df[
        [f"{credit_area}-EnrolledToCloseGap" for credit_area in credit_areas]
    ].sum(axis=1)

    df["TotalExcessCreditsDeficient"] = df[
        [
            f"{credit_area}-Deficiency"
            for credit_area in credit_areas
    ...
    df["TotalExcessCreditsDeficient"] = df["TotalExcessCreditsDeficient"].apply(
        lambda x: max(0, x)
    )

    df["can_catchup?"] = (
        df["TotalExcessCreditsDeficient"] <= df["max_possible_credits_to_recover"]
    )
...
def return_num_of_makeup_credit(year_in_hs, term):
    ...
    return num_of_summers * 3 + num_of_semesters * 1
...
def return_deficiency(student_row, credit_area):
    ...
    credits_On_Track_Needed_by_end_of_term = student_row[
        f"{credit_area}-On_Track_Needed_by_end_of_term"
    ]
