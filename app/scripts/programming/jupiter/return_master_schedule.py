...
def main(request, form):
    ...
    output_lst = []
    for (course,section), course_rows in df.groupby(['JupiterCourse','JupiterSection']):
        ...
        temp_dict = {
            ...
            'JupiterTab':return_jupiter_tab(",".join(course_rows['JupiterPeriod'].unique().tolist())),
...
def return_student_jupiter(request, form):
    ...
    jupiter_df = jupiter_df[['CourseCode','SectionID','JupiterCourse','JupiterSection']]
    ...
    jupiter_output_df = student_schedules_df.merge(jupiter_df, left_on=['Course','Section'], right_on=['CourseCode','SectionID'], how='inner')
    jupiter_output_df = jupiter_output_df[['StudentID','JupiterCourse','JupiterSection']].sort_values(by=['StudentID'])
...
def return_jupiter_schedule(request, form):
    ...
    filename = utils.return_most_recent_report_by_semester(files_df,'MasterSchedule',year_and_semester=year_and_semester)
    master_schedule_df = utils.return_file_as_df(filename)
    print(master_schedule_df)

    # filename = utils.return_most_recent_report_by_semester(files_df,'6_31',year_and_semester=year_and_semester)
    # email_reference_df = utils.return_file_as_df(filename)
    # email_reference_df['DelegatedNickName1'] = email_reference_df['DelegatedNickName']
    # email_reference_df['DelegatedNickName2'] = email_reference_df['DelegatedNickName']

    # print(email_reference_df)

    filename = utils.return_most_recent_report_by_semester(files_df,'6_42',year_and_semester=year_and_semester)
    teacher_reference_df = utils.return_file_as_df(filename)
    teacher_reference_df['TeacherName'] = teacher_reference_df['LastName'] + ' ' + teacher_reference_df['FirstName'].str[0]
    ...
    ## attach Teacher 2 
    teachers_df = student_schedules_df[['Course','Section','Teacher1','Teacher2']].drop_duplicates()
    df = master_schedule_df.merge(teachers_df, left_on=['CourseCode','SectionID'], right_on=['Course','Section'], how='left')
    ...
    for teacher_num in [1,2]:
        df = df.merge(teacher_reference_df[['Teacher',f"DelegatedNickName{teacher_num}"]], left_on=[f'Teacher{teacher_num}'], right_on=[f'Teacher'], how='left')
    # attach period
