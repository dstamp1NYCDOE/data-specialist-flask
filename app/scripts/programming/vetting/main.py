...
def main():
    ...
    regents_max_pvt = pd.pivot_table(regents_max_df, index='StudentID',columns='Exam',values='NumericEquivalent',aggfunc='max')
    ...
    student_department_gpa = pd.pivot_table(cr_1_14_df, index=['StudentID'], columns=['dept'], values='NumericEquivalent', aggfunc='mean')
    student_department_gpa = student_department_gpa[['E','M','S','H']]
    student_department_gpa.columns = ['ELA GPA','Math GPA','Science GPA','Soc Stud GPA']
    ...
    AP_courses_pvt = pd.pivot_table(AP_courses_df, index=['StudentID'], columns=['Course Title'], values='NumericEquivalent', aggfunc='mean')
    ...
    student_on_time_pvt = pd.pivot_table(attd_by_student,index='StudentID',columns='Pd', values='%_on_time',aggfunc='mean')
    student_on_time_pvt.columns = [ f"P{x}_%_on_time" for x in student_on_time_pvt.columns] 

    student_present_pvt = pd.pivot_table(attd_by_student,index='StudentID',columns='Pd', values='%_present',aggfunc='mean')
    student_present_pvt.columns = [ f"P{x}_%_present" for x in student_present_pvt.columns] 


    students_df = students_df.merge(student_present_pvt, on='StudentID', how='left').fillna('')
    students_df = students_df.merge(student_on_time_pvt, on='StudentID', how='left').fillna('')
